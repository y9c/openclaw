...
function migrateLegacyNonCodexRuntimeModelRef(raw: string) {
  const migrated = migrateLegacyRuntimeModelRef(raw);
  if (!migrated || migrated.legacyProvider === LEGACY_CODEX_PROVIDER_ID) {
    return null;
  }
  return migrated;
}
...
function normalizeLegacyRuntimeAgentModelConfig(raw: unknown): {
  ...
  if (typeof raw === "string") {
    const migrated = migrateLegacyNonCodexRuntimeModelRef(raw);
    return migrated
  ...
  const migratedPrimary =
    typeof raw.primary === "string" ? migrateLegacyNonCodexRuntimeModelRef(raw.primary) : null;
  if (!migratedPrimary) {
    return { value: raw, changed: false };
  }
  ...
  if (Array.isArray(raw.fallbacks)) {
    next.fallbacks = raw.fallbacks.map((fallback) => {
      ...
      const migratedFallback = migrateLegacyNonCodexRuntimeModelRef(fallback);
...
function normalizeLegacyRuntimeAllowlistModels(
  ...
  const legacyEntries: Array<[string, unknown]> = [];
  for (const [rawKey, entry] of Object.entries(rawModels)) {
    const migrated = migrateLegacyNonCodexRuntimeModelRef(rawKey);
