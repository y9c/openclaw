...
- Session status output now separates `Runtime:` from `Runner:`. `Runtime` is the process/sandbox execution path (`direct`, `docker/*`), while `Runner` tells you which agent loop is handling the session, such as embedded PI, a CLI-backed provider, or Codex. See [Agent runtimes](/concepts/agent-runtimes) for the provider/model/runtime distinction.
